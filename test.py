...
def open_trace(file_name):
    ...
    trace = Trace(file_name)  # Create a Trace object linked to the file
    #trace.initialize_abf()
    #trace.time, trace.current = abf.sweepX, abf.sweepY
    return trace
...
class Trace():
    def __init__(self, file_name):
        ...
        self.abf = None
        self.time, self.current = None, None  # Initialize time and current as None
        self.segments = []
        self.baseline = None
        self.peaks = []

    def initialize_abf(self):
        if self.abf is None:
            self.abf = pyabf.ABF(self.file_name)
            self.time, self.current = self.abf.sweepX, self.abf.sweepY

    def serialize(self):
        # Before pickling, initialize the abf object
        # Exclude the abf object from pickling by setting it to None
        abf = self.abf
        self.abf = None
        # Serialize the object without the abf attribute
        data = pickle.dumps(self)
        # Restore the abf attribute after pickling
        self.abf = abf
        return data
    ...
    def deserialize(data):
        ...
        # After deserialization, reinitialize the abf object
        trace.initialize_abf()
        return trace
...
class Peak():
    def __init__(self, trace):
        self.file_name = trace.file_name
        self.trace = trace
        self.peak_x = None
        self.peak_y = None
    ...
    def highest_point(self, x_clicked):
        index_clicked = np.argmin(np.abs(self.trace.time - x_clicked))
        search_window = 100 #search window for setting peaks
        
        # Create a 1-second window surrounding the clicked point
        points_per_second = int(1000 * self.trace.abf.dataPointsPerMs) #20 points per Ms given a sampling freq of 20 Hz
        start_index = max(0, index_clicked - search_window)
        end_index = min(len(self.trace.time) - search_window, index_clicked + search_window)
        
        range_y = self.trace.current[start_index:end_index] #change so that it uses plottedtrace
        max_index = np.argmax(abs(range_y))
        index_highest = start_index + max_index
        self.x_highest = self.trace.time[index_highest]
        self.y_highest = self.trace.current[index_highest]
    ...
    def on_line_click(self, event):
        if isinstance(event, PickEvent):
            index = event.ind[0]
            x_clicked = self.trace.time[index]
            y_clicked = self.trace.current[index]
...
class FileManager:
    def __init__(self, file_names, segments, peaks, traces):
    ...
    def update_graph(self):
        ...
        self.trace = open_trace(self.current_file_name)
        self.traces.append(self.trace)
        self.trace.initialize_abf()
        self.time, self.current = self.trace.time, self.trace.current
        #self.traces.append(self.trace)
        ...
        current_peaks = self.get_peaks_for_file(self.current_file_name)
        if current_peaks:
    ...
    def save_traces(self):
        ...
        if filename:
            ...
            print(f"All traces saved as {filename}")

    def load_traces(self):
        filename = filedialog.askopenfilename(filetypes=[('Pickle Files', '*.pickle')])
        if filename:
            with open(filename, 'rb') as file:
                loaded_traces = pickle.load(file)

            # Update the list of traces with the loaded data
            self.traces = loaded_traces
            
            # Update the GUI display and plots accordingly
            self.update_graph()
    ...
    def get_peaks_for_file(self, file_name):
        self.file_peaks = []
        for peak in self.peaks:
            if peak.file_name == file_name:
                self.file_peaks.append(peak)
        return self.file_peaks
    ...
    def setPeak(self, event):
        ...
        self.trace.peaks.append(self.current_peak)
...
def EphysApp():
    ...
    # Add save and load data buttons
    save_data_button = Button(master=root, command=FileMan.save_traces, text='Save Trace Data')
    save_data_button.pack(side="left")

    load_data_button = Button(master=root, command=FileMan.load_traces, text='Load Trace Data')
